...
BINARY_RAW_URL="https://raw.githubusercontent.com/alireza-2030/backhaul-manager/main/dagger-connect/DaggerConnect"
...
download_binary() {
    ...
    if wget -q --show-progress "$BINARY_RAW_URL" -O "$INSTALL_DIR/DaggerConnect"; then
        ...
        if "$INSTALL_DIR/DaggerConnect" -v &>/dev/null; then
            VERSION=$("$INSTALL_DIR/DaggerConnect" -v 2>&1 | grep -oP 'v[\d.]+' || echo "unknown")
