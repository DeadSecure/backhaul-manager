...
install_amnezia() {
    ...
    # 'openresolv' is critical for awg-quick to set up routes/DNS
    DEBIAN_FRONTEND=noninteractive apt-get install -y -qq amneziawg openresolv wireguard-tools
...
install_menu() {
    ...
    while true; do
        ...
        if [[ -n "$REMOTE_PEER_IP" ]]; then break; fi
