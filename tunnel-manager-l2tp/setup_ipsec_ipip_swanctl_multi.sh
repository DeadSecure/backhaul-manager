...
# ==========================================
# IPIP + IPSec (Swanctl) Multi-Tunnel Manager
# Architecture: IPIP over IPSec (Transport Mode)
# Engine: StrongSwan 5.x + Swanctl (VICI)
# Advantages: Lighter than GRE (saves 4 bytes), Same stability.
# ==========================================
...
install_dependencies() {
    ...
    # Kernel Modules for IPIP
    modprobe af_key >> "$LOG_FILE" 2>&1
    modprobe ipip >> "$LOG_FILE" 2>&1   # Changed from ip_gre
...
setup_ipip_interface() {
    ...
    # Using ipip${id} instead of gre${id}
    cat > "/usr/local/bin/ipsec-ipip-up-${id}.sh" <<EOF
#!/bin/bash
...
EOF
    ...
    cat > "/etc/systemd/system/ipsec-ipip-${id}.service" <<EOF
[Unit]
Description=IPIP over IPsec Tunnel ${id}
After=strongswan-swanctl.service network.target

[Service]
Type=oneshot
ExecStart=/usr/local/bin/ipsec-ipip-up-${id}.sh
# Ignore errors if SA is already up
ExecStartPost=-/usr/sbin/swanctl --initiate --child tun${id}
RemainAfterExit=yes
ExecStop=/sbin/ip tunnel del ipip${id}
...
EOF
    ...
    systemctl enable "ipsec-ipip-${id}"
    systemctl restart "ipsec-ipip-${id}"
}
...
install_menu() {
    echo -e "${BLUE}--- Install New IPsec+IPIP Tunnel (Swanctl) ---${NC}"
    ...
    setup_ipip_interface "$TUN_ID" "$SELECTED_LOCAL_IP" "$REMOTE_IP" "$tun_local" "$tun_remote"
...
uninstall_menu() {
    ...
    if [[ -n "$TUN_ID" ]]; then
        ...
        rm -f "/usr/local/bin/ipsec-ipip-up-${TUN_ID}.sh"
...
check_tunnels() {
    echo -e "${BLUE}--- Tunnel Status Check (IPIP) ---${NC}"
    printf "%-5s %-12s %-25s %-20s\n" "ID" "Service" "IPsec SA" "Ping Test (IPIP)"
    echo "------------------------------------------------------------------"
    ...
    systemctl reset-failed "ipsec-ipip-*" >/dev/null 2>&1
    
    for service_file in /etc/systemd/system/ipsec-ipip-*.service; do
        if [ ! -f "$service_file" ]; then continue; fi
        
        [[ $service_file =~ ipsec-ipip-([0-9]+).service ]] && id="${BASH_REMATCH[1]}"
        ...
        elif ip link show "ipip${id}" >/dev/null 2>&1; then
            svc_status="${GREEN}Active (Up)${NC}"
        elif systemctl is-active --quiet "ipsec-ipip-${id}"; then
...
while true; do
    clear
    echo -e "${GREEN}   Swanctl IPIP+IPsec Manager   ${NC}"
    echo "1) Install IPIP Tunnel"
    echo "2) Uninstall IPIP Tunnel"
